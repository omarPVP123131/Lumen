...
- **Novedad v3.5.7:** `FuncMeta.defaults: Vec<Option<DefaultValue>>` (`Int/Float/Str/Bool`) persistidos en el chunk para `bind_args` unificado (`Call`/`CallValue`/`run_function`/hilos usan defaults reales cuando `i>=args.len()` en vez de `Void`/`pop()` corrupto). Ver `docs/produccion.md` §1.3.
- **Magic**: `LUMN` (4 bytes)
...
Ver `docs/produccion.md` checklist final: `cargo test --workspace` (956), `cargo bench`, `lumen check examples` 396/396, `LUMEN_HEADLESS=1` headless, `CHUNK_VERSION 7` con fallback v6.
