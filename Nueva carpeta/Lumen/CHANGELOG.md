...
## [3.94.24] - 2026-09-02 — Caza de bugs con paridad VM⇄AOT (casts, bucles, bits, slices, rangos, ternarios, closures, mapas) + QA A–P + benchmark de todas las áreas

- **Cast real FIX:** `X como T` ya **convierte** en runtime (antes era no-op): `como entero` trunca, `como decimal` promueve, `como booleano` usa truthiness, `como texto` stringifica — paridad VM/AOT (`__cast_entero`/`__cast_decimal`/`__cast_booleano` + `_to_int`/`_to_flt` en `lumen_rt.h`)
- **Foreach FIX:** `continuar`/`romper` dentro de `para … en` ya no dan E055/E070 (sema sube `loop_depth`; IR registra `loop_labels` con `continue_label`)
- **Bitnot FIX:** el plegado unario de constantes **reemplaza in-situ** el operando en vez de duplicarlo — `imprimir("b:", ~8)` ya imprime `b:-9` (antes `8-9`, perdía el prefijo)
- **Slice FIX:** `a[x..y]` / `s[x..y]` (y `..=`) bajan a la builtin `__rodaja` con paridad exacta VM/AOT — antes el AOT-C devolvía 0 / primer carácter
- **Rango-en-variable FIX:** `a[r]` / `s[r]` con `r: lista<entero>` (rango materializado) ya funciona en AOT-C — nueva `_arr_getv` en `lumen_rt.h` (índice entero o rango) cableada en los tres emisores del backend C (camino `PUSH`, camino `estack` y `_lw_arr_get` Cranelift)
- **Ternario inline FIX:** ternarios dentro de argumentos de `imprimir` en bucles ya no dejan un `void` — `JmpIf` hace spill de la pila de expresiones ANTES del salto condicional (el prefijo no se pierde en el camino falso)
- **Closure-captura FIX:** los lambdas `|x| …` / `funcion …` capturan por **snapshot** (paridad VM): se registra `program.parents[lambda]` y `base_bindings` alinea el seed de capturas con la celda global cruda — la mutación dentro del closure ya no altera al definidor y cada closure conserva su estado
- **Mapa FIX/DOC:** `_map_set` restaurado a la implementación de referencia (semántica por valor = paridad VM). Documentado: en AOT-C el mapa es O(n) por inserción y O(n²) de memoria (sin refcounting); para mapas masivos usar la VM (ImMap persistente). `M_mapa_basico` cubre 1 000 claves
- **DOC:** lambdas se escriben `funcion () { … }` o `|x| …` (la flecha `=>` solo es JS dentro de `__js_eval`); `|| { … }` colisiona con `||` lógico
- **QA:** `scripts/regresion_qa.py` ampliado a **A–P (20/20)** con modo `parity` (VM vs binario nativo, salida y exit-code idénticos)
- **Benchmark:** nueva suite `benchmarks/run_bench_all.py` + `benchmarks/c/bench_all.c` con **10 áreas** (fib, sum, primes, strings, arrays, structs, enums, maps, closures, unicode) comparando VM vs AOT-C vs C -O3; resultados en `benchmarks/results/benchmark_all.csv` e `informe_all.md`
- **Verificación:** `cargo fmt --check`/`clippy -D warnings`/`test --workspace` verdes, `lumen check` 396/396, `ci_gate` PASS 392/389 (0 crashes)
- **Docs:** `docs/informes/AUDITORIA_V39424.md` actualizada con los 7 fixes + benchmark
- **Playground/CLI FIX:** `lumen serve` funciona **out-of-the-box sin compilar WASM** — imports de `web/showcase/*.html` corregidos a `../pkg/lumen_wasm.js` + alias del servidor para `.../pkg/<archivo>` (adiós 404 de `/web/showcase/pkg/lumen_wasm.js`); eliminado el aviso engañoso de `lumen_wasm_bg.wasm` (modo híbrido: la VM nativa ejecuta vía `/api/run`); corregido el **bucle de redirección** de `/web/showcase/` (índice de directorio + 404 real en vez de servir el shim de redirección); soporte HEAD, `/api/health` con `mode` y nuevo comando `lumen wasm [build|serve]` (antes `lumen wasm-pack …` daba "Comando desconocido")

## [3.94.23] - 2026-09-02 — Cierre auditoría v3.94.22 (bugs A–K) + regresión QA

- **A FIX:** `dueno T` se auto-desreferencia — `r.id`, `l[0]`, `l.largo()` y operadores atraviesan `Dueno` (igual que ya hacía `Prestado` desde v3.5.7); cubre struct simple, genérico (`dueno Caja<entero>`) y lista de structs
- **B FIX:** API de concurrencia con verificación — `check` valida literales de nombre de función en `hilo_lanzar*`/`mutex_bloquear`/`rwlock_*`/`tarea_*`/`stream_*`/`par_*`/`generador_*`/`scope_*` (E042 si hay typo); en runtime `run_function_or_value` devuelve `error("Función '...' no definida")` en vez de `void` silencioso
- **C FIX:** `en_tiempo_compilacion` — lo no evaluable en tiempo de compilación ahora es **E090** (antes degradaba silencioso a runtime); lo puro sigue plegando
- **E FIX/DOC:** overflow documentado como wraparound intencional + nuevos builtins verificados `__suma_verificada`/`__resta_verificada`/`__multiplicacion_verificada`/`__division_verificada` expuestos en `matematicas.nv` como `resultado<entero, texto>`
- **G FIX:** E031 explica que `cualquiera`/`numero` normaliza a `decimal` y sugiere `(x) como entero`
- **J REFUERZO:** dedup de cascadas del parser por posición exacta + nota "(N error(es) derivado(s) de la misma causa raíz agrupados)"
- **D/H/I/K DOC:** bindings de `si sea` por valor, sintaxis campos vs params, `docs/referencia/concurrencia.md`, `error-codes.md` E090, paralelismo en entornos 1-core
- **CI:** nuevo job `qa-regresion` (`scripts/regresion_qa.py`, 11/11) + gate stdlib 69/69; release-notes con `${TAG}`/`${VER}`/`${REPO}` expandidos (ya no sale "tag" literal)
- **Tests:** unit tests nuevos en `sema.rs` (dueno, reflexión, cualquiera) y `comptime.rs` (pliegue/reporte)
