...
impl Parser {
    ...
    pub fn parse(self) -> (Program, Vec<ParseError>) {
        let (program, errors, _) = self.parse_with_report();
        (program, errors)
    }

    /// Igual que `parse`, pero además devuelve cuántos errores se suprimieron
    /// por cascada (QA bug J) para mostrarlos en el resumen del CLI.
    pub fn parse_with_report(mut self) -> (Program, Vec<ParseError>, usize) {
        ...
        let suppressed = self.suppressed_errors;
        (program, self.errors, suppressed)
    }
    ...
    fn error(
        ...
        // Mitigación de cascada (QA #4, J): los errores derivados del mismo
        // punto ruidoso no aportan — se deduplican y se limita el total.
        const MAX_ERRORS: usize = 20;

        // Dedup: mismo punto exacto (línea + columna), sin importar el código,
        // es síntoma de la misma causa raíz (p. ej. E012 + E020 en la misma
        // expresión rota) — se reporta solo el primero.
        if self.errors.iter().any(|e| e.span.start == span.start) {
