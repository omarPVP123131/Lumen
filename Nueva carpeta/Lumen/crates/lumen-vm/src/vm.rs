...
// 360 nombres (generado por scripts/gen_builtin_names.py)
...
/// Ejecuta `run_function` convirtiendo cualquier error en un `Value::Error`
/// con mensaje legible, en vez de `void` silencioso (QA bug B: la API de
/// hilos/mutex/tareas resolvía el nombre de función por reflexión y un typo
/// se tragaba sin avisar). Los errores quedan visibles como `error(...)`.
fn run_function_or_value(vm: &mut VM, name: &str, args: Vec<Value>) -> Value {
    match vm.run_function(name, args) {
        Ok(v) => v,
        Err(e) => Value::Error(Box::new(Value::str(vm_error_message(&e)))),
    }
}
...
impl VM {
    ...
    fn call_core_builtin(&mut self, name: &str, args: &[Value]) -> Option<Result<(), VmError>> {
        ...
        // Aritmética verificada (QA bug E): variantes que detectan overflow de
        // enteros y devuelven `error(...)` en vez de hacer wraparound silencioso.
        if name == "__suma_verificada" || name == "__checked_add" {
            let a = args.first().and_then(|v| v.as_i64());
            let b = args.get(1).and_then(|v| v.as_i64());
            match (a, b) {
                (Some(x), Some(y)) => match x.checked_add(y) {
                    Some(r) => self.push(Value::Exito(Box::new(Value::Int(r)))),
                    None => self.push(Value::Error(Box::new(Value::str(
                        "Desbordamiento de entero (overflow) en la suma",
                    )))),
                },
                _ => self.push(Value::Error(Box::new(Value::str(
                    "'suma_verificada' espera dos enteros",
                )))),
            }
            return Some(Ok(()));
        }
        if name == "__resta_verificada" || name == "__checked_sub" {
            let a = args.first().and_then(|v| v.as_i64());
            let b = args.get(1).and_then(|v| v.as_i64());
            match (a, b) {
                (Some(x), Some(y)) => match x.checked_sub(y) {
                    Some(r) => self.push(Value::Exito(Box::new(Value::Int(r)))),
                    None => self.push(Value::Error(Box::new(Value::str(
                        "Desbordamiento de entero (overflow) en la resta",
                    )))),
                },
                _ => self.push(Value::Error(Box::new(Value::str(
                    "'resta_verificada' espera dos enteros",
                )))),
            }
            return Some(Ok(()));
        }
        if name == "__multiplicacion_verificada" || name == "__checked_mul" {
            let a = args.first().and_then(|v| v.as_i64());
            let b = args.get(1).and_then(|v| v.as_i64());
            match (a, b) {
                (Some(x), Some(y)) => match x.checked_mul(y) {
                    Some(r) => self.push(Value::Exito(Box::new(Value::Int(r)))),
                    None => self.push(Value::Error(Box::new(Value::str(
                        "Desbordamiento de entero (overflow) en la multiplicación",
                    )))),
                },
                _ => self.push(Value::Error(Box::new(Value::str(
                    "'multiplicacion_verificada' espera dos enteros",
                )))),
            }
            return Some(Ok(()));
        }
        if name == "__division_verificada" || name == "__checked_div" {
            let a = args.first().and_then(|v| v.as_i64());
            let b = args.get(1).and_then(|v| v.as_i64());
            match (a, b) {
                (_, Some(0)) => self.push(Value::Error(Box::new(Value::str("División por cero")))),
                (Some(x), Some(y)) => match x.checked_div(y) {
                    Some(r) => self.push(Value::Exito(Box::new(Value::Int(r)))),
                    None => self.push(Value::Error(Box::new(Value::str(
                        "Desbordamiento de entero (overflow) en la división",
                    )))),
                },
                _ => self.push(Value::Error(Box::new(Value::str(
                    "'division_verificada' espera dos enteros",
                )))),
            }
            return Some(Ok(()));
        }
        ...
        // QA (cast real): conversiones runtime de `X como T`.
        if name == "__cast_entero" {
            let v = args.first().cloned().unwrap_or(Value::Int(0));
            let n = match v {
                Value::Float(f) => Value::Int(f as i64), // trunca hacia cero
        ...
        if name == "__cast_booleano" {
            let v = args.first().cloned().unwrap_or(Value::Bool(false));
            self.push(Value::Bool(v.is_truthy()));
            return Some(Ok(()));
        }
        // QA (slice de rango): `a[x..y]` / `a[x..=y]` sobre listas y texto,
        // con la MISMA semántica que ArrayGet con índice-rango (los negativos
        // saturan a 0, índices fuera de rango se omiten).
        if name == "__rodaja" {
            let container = args.first().cloned().unwrap_or(Value::arr(vec![]));
            let start = args.get(1).and_then(|v| v.as_i64()).unwrap_or(0);
            let end = args.get(2).and_then(|v| v.as_i64()).unwrap_or(0);
            let inclusive = args.get(3).and_then(|v| v.as_bool()).unwrap_or(false);
            let last = if inclusive { end } else { end - 1 };
            let indices: Vec<i64> = if start <= last {
                (start..=last).collect()
            } else {
                Vec::new()
            };
    ...
    fn call_extra_builtin(&mut self, name: &str, args: &[Value]) -> Option<Result<(), VmError>> {
        ...
        if name == "__tarea_lanzar" || name == "__task_spawn" {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                std::thread::spawn(move || {
                    ...
                    let result = run_function_or_value(&mut vm, &fn_name, fn_args);
            ...
            #[cfg(target_arch = "wasm32")]
            {
                ...
                let mut vm = VM::new(bc);
                let result = run_function_or_value(&mut vm, &fn_name, fn_args);
                self.task_results_sync.insert(task_id.clone(), result);
            }
        ...
        if name == "__hilo_lanzar" || name == "__thread_spawn" {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            {
                ...
                let handle = std::thread::spawn(move || {
                    ...
                    run_function_or_value(&mut vm, &fn_name, fn_args)
                });
        ...
        if name == "__mutex_bloquear" || name == "__mutex_lock" {
            ...
            let result = if let Some(mutex) = mtx {
                ...
                vm.channels = Arc::clone(&self.channels);
                vm.mutexes = Arc::clone(&self.mutexes);
                run_function_or_value(&mut vm, &fn_name, vec![fn_arg])
            } else {
                ...
                Value::Array(items) => {
                    ...
                    let mapped: Vec<Value> = items
                        ...
                        .map(|item| {
                            ...
                            run_function_or_value(&mut vm, &fn_name, vec![item.clone()])
                        })
                        .collect();
                    ...
                    let results: Vec<Value> = {
                        ...
                        for item in items.iter() {
                            ...
                            handles.push(std::thread::spawn(move || {
                                ...
                                run_function_or_value(&mut vm, &fn_clone, vec![item_clone])
                            }));
                        }
                        handles
                            .into_iter()
        ...
        if name == "__par_unir" || name == "__par_join" {
            ...
            let (r1, r2) = {
                ...
                let h1 = std::thread::spawn(move || {
                    ...
                    if fn1.is_empty() {
                        ...
                        run_function_or_value(&mut vm, &fn1, vec![a1])
                    }
                });
                let h2 = std::thread::spawn(move || {
                    ...
                    if fn2.is_empty() {
                        ...
                        run_function_or_value(&mut vm, &fn2, vec![a2])
                    }
                });
                (
            ...
            let (r1, r2) = {
                ...
                let v1 = if fn1.is_empty() {
                    ...
                    run_function_or_value(&mut vm1, &fn1, vec![a1])
                };
                let v2 = if fn2.is_empty() {
                    Value::Void
                } else {
                    run_function_or_value(&mut vm2, &fn2, vec![a2])
                };
                (v1, v2)
            };
        ...
        if name == "__generador_siguiente" || name == "__generator_next" {
            ...
            let result = run_function_or_value(&mut vm, &fn_name, vec![val]);
            self.push(result);
            return Some(Ok(()));
        }
        ...
        match op {
            ...
            Opcode::CallValue => {
                ...
                if name == "imprimir" || name == "print" {
                    ...
                    self.push(n);
                } else if name == "__cast_booleano" {
                    let v = args.first().cloned().unwrap_or(Value::Bool(false));
                    self.push(Value::Bool(v.is_truthy()));
                } else if name == "__rodaja" {
                    let container = args.first().cloned().unwrap_or(Value::arr(vec![]));
                    let start = args.get(1).and_then(|v| v.as_i64()).unwrap_or(0);
                    let end = args.get(2).and_then(|v| v.as_i64()).unwrap_or(0);
                    let inclusive = args.get(3).and_then(|v| v.as_bool()).unwrap_or(false);
