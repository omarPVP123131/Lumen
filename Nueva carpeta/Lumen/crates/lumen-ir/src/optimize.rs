            ...
            Instr::Unary(op) => {
                ...
                if let Some(idx) = ia {
                    if let Some(r) = fold_un(op, a) {
                        // QA (bug unary fold): ANTES se hacía out.push(plegado)
                        // sin quitar la constante del operando → la instrucción
                        // original quedaba en el flujo y la pila ganaba un
                        // valor extra (p. ej. `imprimir("b:", ~8)` empujaba
                        // "b:", 8, -9 con argc=2 y se perdía "b:"). Se
                        // REEMPLAZA in-situ la constante para conservar el
                        // modelo de profundidades y los índices de `st`.
                        if idx < out.len()
                            && matches!(
                                out[idx],
