...
impl IRBuilder {
    ...
    fn gen_stmt(&mut self, stmt: &Stmt) {
        match stmt {
            ...
            Stmt::ForEach {
                ...
                // QA: `para-cada` es un ciclo — `romper`/`continuar` deben
                // saltar a las etiquetas correctas (continuar → incrementar).
                self.loop_labels.push(LoopLabels {
                    break_label: end_label,
                    continue_label,
                    loop_name: None,
    ...
    fn gen_expr(&mut self, expr: &Expr) {
        match expr {
            ...
            Expr::Index { expr, index, .. } => {
                // QA (slice de rango): `a[x..y]` / `a[x..=y]` se bajan a la
                // builtin polimórfica `__rodaja` (lista y texto) en vez de
                // materializar el rango como array + ArrayGet. Antes el AOT-C
                // no soportaba el índice-rango (devolvía 0 / primer carácter),
                // divergiendo de la VM.
                if let Expr::Range {
                    start,
                    end,
                    inclusive,
                ...
                } = index.as_ref()
                {
                    ...
                    self.gen_expr(start);
                    self.gen_expr(end);
                    self.emit(Instr::ConstBool(*inclusive));
                    // 4 argumentos: (contenedor, inicio, fin, inclusivo)
                    self.emit(Instr::Call("__rodaja".to_string(), 4));
                } else {
                    self.gen_expr(expr);
                    self.gen_expr(index);
                    self.emit(Instr::ArrayGet);
            ...
            Expr::Cast {
                ...
                // QA (cast real): `X como T` ahora CONVIERTE en runtime.
                // Antes era un no-op (el valor pasaba tal cual), lo que
                // contradecía la documentación: "(x) como entero si necesitas
                // truncar". Se baja a builtins de conversión compartidos por
                // VM y AOT-C (paridad garantizada).
                self.gen_expr(expr);
                match cast_type {
                    Type::Entero => self.emit(Instr::Call("__cast_entero".to_string(), 1)),
                    Type::Decimal => self.emit(Instr::Call("__cast_decimal".to_string(), 1)),
                    Type::Booleano => self.emit(Instr::Call("__cast_booleano".to_string(), 1)),
                    Type::Texto => self.emit(Instr::Call("a_texto".to_string(), 1)),
                    // Numero, Lista, Struct, etc.: normalización de tipo en
                    // sema; el valor numérico ya está normalizado.
                    _ => {}
                }
            }
            Expr::StructInit {
    ...
    fn compile_lambda(&mut self, params: &[Param], body: &[DeclOrStmt]) -> String {
        ...
        // v3.94.24 (bug cierre lambda captura): registra la relación
        // padre→hijo del lambda para que compute_captures (backend C) y la
        // resolución de capturas encuentren al definidor. Antes los lambdas
        // no aparecían en program.parents y la captura se resolvía como
        // celda global COMPARTIDA (mutación visible en el definidor, a
        // diferencia de la VM que captura por valor/snapshot).
        if let Some(parent) = &self.current_func {
            self.program
                .parents
                .insert(lambda_name.clone(), parent.clone());
        }
