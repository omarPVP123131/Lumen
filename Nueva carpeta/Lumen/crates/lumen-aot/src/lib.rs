...
int64_t _lw_arr_get(int64_t a, int64_t i) { return _lw_h(_arr_getv(_lw_u(a), _lw_u(i))); }
    ...
    fn base_bindings(
        ...
        for ins in &func.instrs {
            match ins {
                ...
                Instr::StoreLocal(n) => {
                    let at_base = scopes.len() == 1;
                    let top = scopes.last_mut().expect("scope base siempre presente");
                    if !top.contains_key(n) {
                        // v3.94.24 (bug cierre lambda): si esta función es la
                        // entrada y `n` es una GLOBAL (capturada por anidadas),
                        // su key es el nombre crudo (celda gv compartida), no
                        // "{fn}::{n}#k". Antes el seed de captura apuntaba a la
                        // key con contador y el lambda escribía OTRA celda
                        // distinta de la del definidor (divergencia VM).
        ...
        for fname in pending {
            ...
            while let Some(anc) = cur {
                if let Some(anc_func) = program.funcs.get(&anc) {
                    ...
                    for (var, key) in
                        base_bindings(&anc, anc_func, &anc_pr, &global_names, is_entry_fn(&anc))
                    {
    ...
    while i < instrs_all.len() {
        ...
        match &instrs_all[i] {
            ...
            Instr::JmpIf(t) if !estack.is_empty() => {
                ...
                // v3.94.24 (bug ternario en bucle): los valores PENDIENTES de la
                // pila de expresiones (args ya evaluados antes de un ternario,
                // p. ej. el prefijo de un `imprimir`) deben spill-earse ANTES de
                // emitir el salto condicional. Antes quedaban en la estack y el
                // spill posterior los emitía DENTRO de la rama verdadera, con lo
                // que el camino falso perdía esos valores (salida "void").
                xe_spill(&mut s, &mut estack, &handler_labels, fn_ret_int);
            ...
            Instr::ArrayGet if estack.len() >= 2 => {
                ...
                // v3.94.24 (bug rango-en-variable): el índice se boxea SIEMPRE y
                // `_arr_getv` decide int vs rango (antes se leía `.i` del rango).
                let b = if kb == 1 { format!("_v_int({})", b) } else { b };
                // elemento del array → NO fresco (alias del contenedor)
                estack.push((format!("_arr_getv({}, {})", a, b), true, false, 0, false));
            }
        ...
        match instr {
            ...
            Instr::Call(n, argc) => {
                ...
                } else if n == "__cast_entero" {
                    s.push_str("  PUSH(_to_int(POP()));\n");
                } else if n == "__cast_decimal" {
                    s.push_str("  PUSH(_to_flt(POP()));\n");
                } else if n == "__cast_booleano" {
                    s.push_str("  PUSH(_v_bool(_truthy(POP())));\n");
                } else if n == "__rodaja" {
                    s.push_str(
                        "  { Val _inc = POP(); Val _e = POP(); Val _s = POP(); Val _c = POP(); PUSH(_rodaja(_c, _s, _e, _inc)); }\n",
                    );
            ...
            Instr::ArrayGet => {
                // v3.94.24 (bug rango-en-variable): `_arr_getv` admite índice
                // entero O rango (lista<entero>) con paridad total con la VM;
                // `_arr_get(_a, _i.i)` coaccionaba el rango a 0 (primer elem).
                s.push_str("  { Val _i = POP(); Val _a = POP(); PUSH(_arr_getv(_a, _i)); }\n")
            }
            Instr::ArraySet => {
