...
LW_HOT Val _to_int(Val a) {
  a = _deref(a);
  if (a.t == T_FLT) return _v_int((int64_t)a.f); /* trunca hacia cero */
  if (a.t == T_INT) return a;
  if (a.t == T_BOL) return _v_int(a.i != 0 ? 1 : 0);
  return _v_int(0);
}
LW_HOT Val _to_flt(Val a) {
  a = _deref(a);
  if (a.t == T_FLT) return a;
  if (a.t == T_INT) return _v_flt((double)a.i);
  if (a.t == T_BOL) return _v_flt(a.i != 0 ? 1.0 : 0.0);
  return _v_flt(0.0);
}
...
/* QA (slice de rango): `a[x..y]` / `a[x..=y]` sobre texto y listas, con la
   MISMA semántica que la VM (ArrayGet con índice-rango): los negativos
   saturan a 0 y los índices fuera de rango se omiten. */
static Val _arrn(Val* xs, int n); /* forward (definida más abajo) */
static Val _rodaja(Val c, Val s0, Val e0, Val inc) {
  c = _deref(c);
  int64_t start = (int64_t)_asf(s0);
  int64_t end = (int64_t)_asf(e0);
  int inclusive = _truthy(inc);
  int64_t last = inclusive ? end : end - 1;
  if (c.t == T_STR) {
    const char* cs = c.s ? c.s : "";
    ...
    char* buf = (char*)malloc((size_t)(cnt * 4 + 1));
    if (!buf) return _v_str("");
    int64_t o = 0;
    if (start <= last) {
      for (i = start; i <= last; i++) {
        int64_t u = i < 0 ? 0 : i;
        if (u >= n) continue;
        const unsigned char* q = (const unsigned char*)cs;
        unsigned cp = 0; int L = 1; int64_t j = 0;
        while (*q && j < u) { L = _utf8_decode(q, &cp); q += L; j++; }
        L = _utf8_decode(q, &cp);
        int el = _utf8_encode(cp, buf + o);
        o += el;
      }
    }
    buf[o] = 0;
    return _v_str_take(buf);
  }
  if (c.t == T_ARR) {
    int64_t n = c.argc;
    int64_t cap = (start <= last && last >= start) ? (last - start + 1) : 0;
    Val* out = (Val*)malloc(sizeof(Val) * (size_t)cap);
    int64_t o = 0, i;
    if (start <= last) {
      for (i = start; i <= last; i++) {
        int64_t u = i < 0 ? 0 : i;
        if (u < n) out[o++] = _dcp(c.items[u]);
      }
    }
    return _arrn(out, (int)o);
  }
  return c;
}
...
/* v3.94.24 (bug rango-en-variable): indexado con un VALOR, no un entero crudo.
   La VM acepta `a[r]` / `s[r]` donde `r` es una variable con un rango
   (lista<entero>): para cada entero del rango se selecciona ese índice (los
   fuera de rango se omiten). El backend C coaccionaba el rango a entero con
   `_i.i` (== 0) y devolvía el primer elemento. */
static Val _arr_getv(Val a, Val ix) {
  if (ix.t == T_ARR) {
    if (a.t == T_STR) {
      const char* cs = a.s ? a.s : "";
      int64_t n = (int64_t)_utf8_len(cs);
      char* buf = (char*)malloc((size_t)(ix.argc * 4 + 1));
      if (!buf) return _v_str("");
      int64_t o = 0;
      for (int k = 0; k < ix.argc; k++) {
        int64_t u = (int64_t)_asf(ix.items[k]);
        if (u < 0 || u >= n) continue;
        const unsigned char* p = (const unsigned char*)cs;
        unsigned cp = 0; int L = 1; int64_t j = 0;
        while (*p && j < u) { L = _utf8_decode(p, &cp); p += L; j++; }
        L = _utf8_decode(p, &cp);
        o += _utf8_encode(cp, buf + o);
      }
      buf[o] = 0;
      return _v_str_take(buf);
    }
    if (a.t == T_ARR) {
      Val* out = (Val*)malloc(sizeof(Val) * (size_t)(ix.argc > 0 ? ix.argc : 1));
      int64_t o = 0;
      for (int k = 0; k < ix.argc; k++) {
        int64_t u = (int64_t)_asf(ix.items[k]);
        if (u >= 0 && u < a.argc) out[o++] = _dcp(a.items[u]);
      }
      return _arrn(out, (int)o);
    }
    return _arr_get(a, 0);
  }
  return _arr_get(a, (int64_t)_asf(ix));
}
...
static Val _map_set(Val m, Val k, Val x) {
  /* NOTA (limitación documentada): este mapa es de VALOR (copia en cada
     asignación vía _dcp), por lo que `d = __map_poner(d, ...)` en bucle es
     O(n) por inserción en el backend C (la VM usa ImMap persistente, O(log
     n)). El buffer anterior no se libera (sin recuento de referencias), así
     que mapas muy grandes en AOT-C consumen O(n²) de memoria — usar la VM
     para mapas masivos. Ver AUDITORIA_V39424.md, ítem 8. */
  Val nv = m;
  int n = m.argc;
