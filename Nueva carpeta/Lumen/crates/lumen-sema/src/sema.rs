...
impl SemanticAnalyzer {
    ...
    /// Posiciones de los argumentos que son NOMBRES DE FUNCIÓN (texto) en las
    /// APIs de reflexión de `stdlib/concurrencia.nv` (QA bug B). Los nombres
    /// importados llegan con prefijo de módulo (ej. `concurrencia_hilo_lanzar`),
    /// así que se compara por igualdad o por sufijo `_{nombre}`. Devuelve las
    /// posiciones y el nombre base (sin prefijo) para los mensajes.
    fn reflection_fn_arg_positions(callee: &str) -> (&'static [usize], &'static str) {
        fn matches_api(callee: &str, name: &str) -> bool {
            callee == name
                || (callee.len() > name.len() + 1
                    && callee.ends_with(name)
                    && callee.as_bytes()[callee.len() - name.len() - 1] == b'_')
        }
        let pos0: &[&str] = &[
            "hilo_lanzar",
            "thread_spawn",
        ...
        if let Some(n) = pos0.iter().find(|n| matches_api(callee, n)) {
            return (&[0], n);
        }
        let pos1: &[&str] = &[
            "mutex_bloquear",
        ...
        if let Some(n) = pos1.iter().find(|n| matches_api(callee, n)) {
            return (&[1], n);
        }
        if let Some(n) = ["par_unir", "par_join"]
            .iter()
            .find(|n| matches_api(callee, n))
        {
            return (&[0, 2], n);
        }
        (&[], "")
    ...
    /// Emite un error E042 si un literal de texto pasado a una API de
    /// reflexión nombra una función que no existe en el programa.
    fn validate_reflection_targets(&mut self, callee: &str, args: &[Expr]) {
        let (positions, api_name) = Self::reflection_fn_arg_positions(callee);
        if positions.is_empty() || !self.functions.contains_key(callee) {
            return;
        }
        for &pos in positions {
            if let Some(Expr::Str { value, span }) = args.get(pos) {
                if value.is_empty() {
                    continue;
                }
                if !self.functions.contains_key(value) {
                    self.errors.push(SemError {
                        code: "E042".to_string(),
                        message: format!(
                            "La función '{}' (pasada como texto a '{}') no está definida",
                            value, api_name
                        ),
                        span: *span,
                        suggestion: format!(
                            "Revisa el nombre (¿typo?). Define '{}' o corrige la llamada.",
                            value
                        ),
                    });
                }
            ...
            Decl::Variable {
                ...
                if init.is_some() {
                    ...
                    if !can_assign(&declared_type, &init_type) {
                        // QA bug G: cuando `cualquiera`/`numero` colapsa a
                        // `decimal`, explicar el porqué en vez de solo nombrar
                        // el tipo resultante.
                        let (message, suggestion) = if declared_type == TypeInfo::Entero
                            && init_type == TypeInfo::Decimal
                        {
                            (
                                    format!(
                                        "No puedes asignar un valor de tipo '{:?}' a una variable de tipo '{:?}'. Nota: `cualquiera`/`numero` normaliza los números a `decimal`",
                                        init_type, declared_type
                                    ),
                                    "Convierte explícitamente con `(x) como entero` o declara la variable como `decimal`".to_string(),
                                )
                        } else {
                            (
                                    format!("No puedes asignar un valor de tipo '{:?}' a una variable de tipo '{:?}'", init_type, declared_type),
                                    format!("Usa un valor de tipo '{:?}' en lugar de '{:?}'", declared_type, init_type),
                                )
                        };
                        self.errors.push(SemError {
                            code: "E031".to_string(),
    ...
    fn analyze_stmt(&mut self, stmt: &Stmt) -> TypeInfo {
        match stmt {
            ...
            Stmt::ForEach {
                ...
                // QA: `para-cada` ES un ciclo — habilitar `romper`/`continuar`
                // dentro del cuerpo (antes daba E055/E070 por no subir loop_depth).
                self.loop_depth += 1;
                for node in body {
                    self.analyze_decl_or_stmt(node);
                }
    ...
    fn analyze_expr(&mut self, expr: &Expr) -> TypeInfo {
        match expr {
            ...
            Expr::Binary {
                ...
                // prestado mut/dueno (bug #6, A): los operadores ven el tipo interior
                let lt = match self.analyze_expr(left) {
                    TypeInfo::Prestado { inner, .. } | TypeInfo::Dueno(inner) => *inner,
                    other => other,
                };
            ...
            Expr::Call {
                ...
                match callee_inner {
                    Expr::Ident { name, .. } => {
                        ...
                        // QA bug B: verificar en `check` los nombres de función
                        // pasados como texto a APIs de reflexión (hilo_lanzar,
                        // mutex_bloquear, …) para que un typo no falle en silencio.
                        self.validate_reflection_targets(&callee, args);
                        ...
                        match func_info {
                            ...
                            None => {
                                ...
                                } else if callee == "__suma_verificada"
                                    || callee == "__resta_verificada"
                                    || callee == "__multiplicacion_verificada"
                                    || callee == "__division_verificada"
                                    || callee == "__checked_add"
                                    || callee == "__checked_sub"
                                    || callee == "__checked_mul"
                                    || callee == "__checked_div"
                                {
                                    // Aritmética verificada (QA bug E): devuelve
                                    // resultado<entero, texto> y detecta overflow.
                                    if args.len() != 2 {
                                        self.errors.push(SemError {
                                            code: "E040".to_string(),
                                            message: format!(
                                                "'{}' espera 2 argumentos enteros, no {}",
                                                callee,
                                                args.len()
                                            ),
                                            span: *span,
                                            suggestion: "Pasa 2 enteros".to_string(),
                                        });
                                    }
                                    for got in arg_types.iter() {
                                        if *got != TypeInfo::Entero && *got != TypeInfo::Numero {
                                            self.errors.push(SemError {
                                                code: "E041".to_string(),
                                                message: format!(
                                                    "'{}' espera 'entero', no '{:?}'",
                                                ...
                                                span: *span,
                                                suggestion: "Pasa valores de tipo entero"
                                                    .to_string(),
                                            });
                                        }
                                    }
            ...
            Expr::Index { expr, index, span } => {
                let raw_type = self.analyze_expr(expr);
                // Auto-deref Prestado<T>/Dueno<T> → T para indexación (QA bug #6, A)
                let expr_type = match &raw_type {
            ...
            Expr::MethodCall {
                ...
                let raw_type = self.analyze_expr(expr);
                // Auto-deref Prestado<T>/Dueno<T> → T para llamadas a métodos (QA bug #6, A)
                let expr_type = match &raw_type {
                    TypeInfo::Prestado { inner, .. } | TypeInfo::Dueno(inner) => (**inner).clone(),
                    _ => raw_type.clone(),
                };
            ...
            Expr::FieldAccess { expr, field, span } => {
                let expr_type = self.analyze_expr(expr);
                // Auto-deref Prestado<T>/Dueno<T> → T para acceso a campos (QA bug #6, A)
...
elegir (c) {
    ...
    // ── QA bug A: `dueno T` se auto-desreferencia para campo e indexación ──
    #[test]
    fn test_dueno_field_access_ok() {
        let src = "estructura Recurso { id: entero }
funcion vacio consumir(dueno Recurso r) {
    imprimir(a_texto(r.id));
}";
        let errors = analyze(src);
        assert!(errors.is_empty(), "Errors: {:?}", errors);
    }
    ...
    #[test]
    fn test_dueno_generic_field_access_ok() {
        let src = "estructura Caja<T> { valor: T }
funcion vacio consumir(dueno Caja<entero> c) {
    ...
    #[test]
    fn test_dueno_list_index_ok() {
        let src = "estructura Item { valor: entero }
funcion vacio consumir(dueno lista<Item> l) {
    imprimir(a_texto(l[0].valor));
}";
        let errors = analyze(src);
        assert!(errors.is_empty(), "Errors: {:?}", errors);
    }

    // ── QA bug B: typo en nombre de función pasado como texto se detecta ────
    #[test]
    fn test_reflection_target_typo_detected() {
        // `hilo_lanzar1` se define localmente para que el checker lo conozca
        let src = "funcion cualquiera hilo_lanzar1(texto fn, cualquiera a1) { retornar a1; }
funcion entero principal() {
    cualquiera h = hilo_lanzar1(\"tarea_pessada\", 10);
}";
        let errors = analyze(src);
        assert!(!errors.is_empty());
        assert_eq!(errors[0].code, "E042");
        assert!(
            errors[0].message.contains("tarea_pessada"),
            "msg: {}",
            errors[0].message
        );
    }
    ...
    #[test]
    fn test_reflection_target_ok() {
        let src = "funcion cualquiera hilo_lanzar1(texto fn, cualquiera a1) { retornar a1; }
funcion cualquiera tarea(entero n) { retornar n; }
funcion entero principal() {
    cualquiera h = hilo_lanzar1(\"tarea\", 10);
}";
        let errors = analyze(src);
        assert!(errors.is_empty(), "Errors: {:?}", errors);
    }

    // ── QA bug G: mensaje E031 explica la normalización cualquiera→decimal ──
    #[test]
    fn test_cualquiera_collapse_message_explains() {
        let src = "funcion cualquiera identidad(cualquiera x) { retornar x; }
funcion entero principal() {
    entero c = identidad(42) + 1;
        ...
        assert_eq!(errors[0].code, "E031");
        assert!(
            errors[0].message.contains("normaliza"),
            "msg: {}",
            errors[0].message
        );
    }
