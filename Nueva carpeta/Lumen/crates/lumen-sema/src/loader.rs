...
fn parse_source(source: &str, path: &Path) -> Result<Program, ModuleError> {
    ...
    let (program, parse_errors, suppressed) = parser.parse_with_report();
    if !parse_errors.is_empty() {
        let mut details: Vec<String> = parse_errors
            .iter()
            .map(|e| {
                format!(
                    "{} [{}:{}]: {} ({})",
        ...
        if suppressed > 0 {
            details.push(format!(
                "({} error(es) derivado(s) de la misma causa raíz agrupados)",
                suppressed
            ));
        }
