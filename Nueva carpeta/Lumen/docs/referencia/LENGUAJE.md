...
> 🧭 **Guía práctica verificada (v3.94.24):** para ejemplos mínimos que compilan
> y corren idéntico en VM y binario nativo (structs, enums, mapas, closures,
> rangos, casts, unicode…), consulta **[GUIA_LENGUAJE.md](GUIA_LENGUAJE.md)**.
...
> **Convención de sintaxis (documentado, QA bug H)**: los **campos de struct**
> se declaran `nombre: Tipo`, mientras que los **parámetros de función** aceptan
> ambos órdenes — `Tipo nombre` (estilo C) o `nombre: Tipo` (estilo struct):

```lumen
estructura Punto { x: entero, y: entero }        // campos: SIEMPRE nombre: Tipo
funcion entero suma(entero a, entero b) { ... }  // params: Tipo nombre
funcion entero resta(a: entero, b: entero) { ... } // params: nombre: Tipo (también válido)
```
...
> **`cualquiera` y normalización numérica (documentado, QA bug G)**:
> `cualquiera` (tipo dinámico) normaliza internamente los números a un tipo
> numérico genérico. Al mezclarlo con un entero, el resultado se promueve a
> `decimal`, por lo que asignarlo a `entero` es un error. El mensaje E031 lo
> explica: "Nota: `cualquiera`/`numero` normaliza los números a `decimal`".
> Convierte explícitamente con `(x) como entero` si necesitas truncar.

```lumen
funcion cualquiera identidad(cualquiera x) { retornar x; }
entero c = identidad(42) + 1;   // ✗ E031: cualquiera→decimal; usa (x) como entero
```
...
> **Auto-desreferenciación de `dueno` (v3.94.23, QA bug A)**: `dueno T` es un
> wrapper de titularidad a nivel de tipos. El checker lo atraviesa automáticamente
> para **acceso a campos** (`r.id`), **indexación** (`l[0]`), **llamadas a
> métodos** (`l.largo()`) y **destructuración de enums** (`si sea E::V(x) = e`).
> Funciona con structs simples, structs genéricos (`dueno Caja<entero>`) y listas.
...
> **Bindings de patrón son por valor (v3.94.23, documentado — QA bug D)**:
> en `si sea algun(c) = cfg { ... }`, la variable `c` es una **copia por valor**
> del contenido. Mutar `c.nivel` dentro del bloque NO modifica `cfg`. Para mutar
> el original usa `prestado mut` en una función. No existe sintaxis de patrón
> por referencia (`si sea algun(mut c) = ...`) — limitación explícita del diseño.

```lumen
opcion<Config> cfg = algun(Config { nivel: 1 });
si sea algun(c) = cfg {
    c.nivel = 5;   // no persiste: c es una copia
}
// cfg.nivel sigue siendo 1
```
...
- Límites: profundidad 128, 1M pasos.
- **Comportamiento v3.94.23 (QA bug C)**: si el bloque **no se puede evaluar en
  tiempo de compilación** (contiene efectos secundarios, variables de runtime,
  límites excedidos), el compilador emite el error **E090** en vez de degradar
  silenciosamente a runtime. `comptime` siempre tiene semántica real de
  evaluación anticipada.

```lumen
funcion entero con_efecto() { imprimir("lado"); retornar 42; }
entero x = en_tiempo_compilacion { con_efecto() };  // ✗ E090: no evaluable
```
...
## 5b. Desbordamiento de Enteros y Aritmética Verificada (v3.94.23, QA bug E)

Los enteros de LÚMEN son `i64`. Por diseño, las operaciones `+`, `-`, `*`, `/`
hacen **wraparound silencioso** ante desbordamiento (igual que C y que Rust en
modo *release*):

```lumen
entero max_val = 9223372036854775807;   // i64::MAX
entero salida  = max_val + 1;
imprimir(a_texto(salida));              // -9223372036854775808 (wraparound)
```

Esto es un **comportamiento intencional y documentado** (no un error), pero
contrasta con la filosofía de "fallar ruidosamente" que sí aplica la división
por cero y los índices fuera de rango. Para quien necesite **detectar** el
desbordamiento, la stdlib (`matematicas.nv`) ofrece variantes verificadas que
devuelven `resultado<entero, texto>`:

```lumen
importar "matematicas.nv";

imprimir(suma_verificada(10, 20));                      // exito(30)
imprimir(suma_verificada(9223372036854775807, 1));      // error(Desbordamiento ...)
imprimir(multiplicacion_verificada(6, 7));              // exito(42)
imprimir(division_verificada(1, 0));                    // error(División por cero)
```

| Función | Detección |
|---|---|
| `suma_verificada(a, b)` | desbordamiento en suma |
| `resta_verificada(a, b)` | desbordamiento en resta |
| `multiplicacion_verificada(a, b)` | desbordamiento en producto |
| `division_verificada(a, b)` | división por cero o desbordamiento (MIN / -1) |
