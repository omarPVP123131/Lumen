...
fn real_main(args: Vec<String>) {
    ...
    match config.command.as_str() {
        ...
        "wasm" | "wasm-pack" | "build-wasm" => {
            print_wasm_help(if config.file.is_empty() {
                "info"
            } else {
                &config.file
            });
...
fn compile_source(path: &str, lib_dirs: &[PathBuf]) -> Bytecode {
    ...
    // comptime (bug #7, C): plegar expresiones comptime evaluables antes de
    // sema. Lo que NO se puede evaluar en tiempo de compilación es un error
    // explícito (antes degradaba silenciosamente a runtime).
    lumen_ir::comptime::ComptimeEvaluator::new(&program).rewrite_program(&mut program);
    let unfolded = lumen_ir::comptime::find_unfolded_comptime(&program);
    if !unfolded.is_empty() {
        let errs: Vec<lumen_sema::SemError> = unfolded
            .into_iter()
            .map(|span| lumen_sema::SemError {
                code: "E090".to_string(),
                message: "`en_tiempo_compilacion` requiere una expresión evaluable en tiempo de compilación (solo literales y funciones puras, sin efectos secundarios ni variables de runtime); no se pudo evaluar en compilación.".to_string(),
                span,
                suggestion: "Usa literales y llamadas a funciones puras dentro del bloque comptime.".to_string(),
            })
            .collect();
        show_sema_errors(&errs, &source, path);
        process::exit(1);
    }
...
fn run_tests(path: &str, lib_dirs: &[PathBuf]) {
    ...
    // comptime (bug #7, C): plegar evaluables; lo no evaluable es error
    lumen_ir::comptime::ComptimeEvaluator::new(&flat).rewrite_program(&mut flat);
    let unfolded = lumen_ir::comptime::find_unfolded_comptime(&flat);
    if !unfolded.is_empty() {
        let errs: Vec<lumen_sema::SemError> = unfolded
            .into_iter()
            .map(|span| lumen_sema::SemError {
    ...
    loop {
        ...
        match trimmed {
            ...
            s if s.starts_with("b ") || s.starts_with("break ") => {
                let rest = if let Some(stripped) = s.strip_prefix("break ") {
                    stripped
            ...
            s if s.starts_with("print ") || s.starts_with("p ") => {
                let var_name = if let Some(stripped) = s.strip_prefix("print ") {
                    stripped
...
fn handle_api_request(
    ...
    match path {
        "/api/health" => {
            let real_wasm = root.join("pkg/lumen_wasm_bg.wasm").is_file()
                || root.join("web/pkg/lumen_wasm_bg.wasm").is_file();
            let mode = if real_wasm { "wasm" } else { "hybrid" };
            json_ok(
                stream,
                &format!(
                    "{{\"status\":\"ok\",\"version\":\"{}\",\"wasm\":{},\"mode\":\"{}\"}}",
                    VERSION, real_wasm, mode
                ),
            );
            true
        }
...
fn handle_http_request(stream: &mut std::net::TcpStream, root: &Path) {
    ...
    let is_asset = raw_rel.ends_with(".js")
        ...
        || raw_rel.ends_with(".ico");

    let repo_root = find_repo_root().unwrap_or_else(|| root.to_path_buf());
    let pkg_stripped = raw_rel.strip_prefix("pkg/").unwrap_or(raw_rel);
    // Cola tras el último segmento "pkg/": permite resolver `.../pkg/<archivo>`
    // (p. ej. `web/showcase/pkg/lumen_wasm.js` → `web/pkg/lumen_wasm.js`) para que
    // las referencias antiguas o rutas relativas no den 404.
    let pkg_tail = raw_rel
        .rsplit_once("pkg/")
        .map(|(_, tail)| tail.to_string());

    let mut candidate_paths: Vec<PathBuf> = if is_asset {
        vec![
            root.join(raw_rel),
    ...
    if is_asset {
        if let Some(tail) = &pkg_tail {
            candidate_paths.push(root.join("web/pkg").join(tail));
            candidate_paths.push(root.join("pkg").join(tail));
            candidate_paths.push(repo_root.join("crates/lumen-wasm/web/pkg").join(tail));
            candidate_paths.push(repo_root.join("crates/lumen-wasm/pkg").join(tail));
        }
    }
    // Índices de directorio: `/web/showcase/` → `/web/showcase/index.html`
    if raw_rel.ends_with('/') {
        candidate_paths.push(root.join(raw_rel).join("index.html"));
        candidate_paths.push(root.join("web").join(raw_rel).join("index.html"));
    }
    ...
    for cand in &candidate_paths {
        ...
        // Directorio (con o sin barra final): servir su index.html. Evita que
        // rutas como `/web/showcase/` caigan en el shim de redirección.
        if cand.is_dir() {
            let idx = cand.join("index.html");
            if idx.is_file() {
                found_file = Some(idx);
                break;
            }
...
/// Guía para el flujo de trabajo WASM. `lumen wasm` / `lumen wasm-pack` ya no
/// fallan como "comando desconocido": explican cómo usar el playground sin
/// compilar nada y cómo generar el artefacto .wasm real (opcional).
fn print_wasm_help(sub: &str) {
    match sub {
        "build" | "compilar" | "compile" => {
            println!();
            println!("  ⚙️  COMPILAR EL ARTEFACTO WASM REAL (opcional)");
            println!("  ═══════════════════════════════════════════════");
            println!("  El playground ('lumen serve') NO requiere este paso: usa el shim");
            println!("  JavaScript y ejecuta el código en la VM nativa del servidor.");
            println!();
            println!("  Para producir lumen_wasm_bg.wasm real necesitas wasm-pack y el");
            println!("  toolchain wasm32-unknown-unknown:");
            println!();
            println!("      rustup target add wasm32-unknown-unknown");
            println!("      cargo install wasm-pack");
            println!("      wasm-pack build crates/lumen-wasm --target web");
            println!();
            println!("  Alternativa del repo (WebAssembly + WASI):");
            println!("      ./crates/lumen-wasm/build-wasi.sh     (Linux / macOS)");
            println!("      .\\crates\\lumen-wasm\\build-wasi.ps1    (Windows)");
            println!();
            println!("  El artefacto se genera en crates/lumen-wasm/pkg/.");
            println!();
            println!("  ⚠  wasm-pack solo funciona desde el código fuente del repo (requiere");
            println!("     crates/lumen-wasm/Cargo.toml). La versión distribuida de LÚMEN ya");
            println!("     incluye el shim JS y NO necesita este paso para usar el playground.");
        }
        "serve" | "playground" => {
            println!("  El playground web se inicia con: lumen serve [--port 8080]");
            println!("  No necesita compilación WASM previa (modo híbrido vía /api/run).");
        }
        _ => {
            println!();
            ...
            println!();
            println!("  🚀 No necesitas compilar nada para probar LÚMEN en el navegador.");
        }
    }
}
...
fn serve_playground(port: u16) -> ! {
    ...
    // El playground funciona sin compilar WASM: el shim JS (web/pkg/lumen_wasm.js)
    // delega la ejecución a /api/run en este servidor (modo híbrido). Solo se
    // avisa si ni siquiera el shim está disponible.
    let shim_js = root.join("web/pkg/lumen_wasm.js");
    let real_wasm = root.join("web/pkg/lumen_wasm_bg.wasm").is_file()
        || root.join("pkg/lumen_wasm_bg.wasm").is_file();
    if !real_wasm {
        eprintln!("ℹ  Modo híbrido: la ejecución usa la VM nativa del servidor (/api/run); no necesitas compilar WASM.");
        eprintln!("   Para generar un artefacto .wasm real: 'lumen wasm build' o 'wasm-pack build crates/lumen-wasm --target web'.");
    }
    if !shim_js.is_file() {
        eprintln!("Aviso: no se encontró web/pkg/lumen_wasm.js (shim del playground). El editor no podrá ejecutar código.");
    }
