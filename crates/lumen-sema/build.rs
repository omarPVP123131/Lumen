// Build script: embebe la stdlib de LÚMEN (stdlib/*.nv) en lumen-sema, que es
// quien resuelve los imports. Asi TODO consumidor del loader (CLI, WASM, API)
// hereda una stdlib que viaja dentro del binario (BUG-152).
// Genera OUT_DIR/embedded_stdlib.rs con `STDLIB_FILES: &[(&str, &str)]`.
// Recompilar incluye automaticamente la stdlib vigente del repo.
use std::env;
use std::fs;
...
fn main() {
    ...
    // crates/lumen-sema → raíz del repo → stdlib/
